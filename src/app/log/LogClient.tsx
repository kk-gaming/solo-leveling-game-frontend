      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map((c) => (
              <button type="button" key={c} onClick={() => setCategory(c)} className={`sl-chip ${category === c ? "active" : ""}`}>
                {c}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <input type="number" min={0} value={duration} onChange={(e) => setDuration(Number(e.target.value))} className="w-full border rounded-md px-3 py-2" />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">Intensity</label>
          <div className="sl-segment">
            {[1, 2, 3, 4, 5].map((i) => (
              <button type="button" key={i} data-active={intensity === i} onClick={() => setIntensity(i)}>
                {i}
              </button>
            ))}
          </div>
